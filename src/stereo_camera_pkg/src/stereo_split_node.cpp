...
public:
    StereoSplitNodeYuyv() : Node("stereo_split_node_yuyv")
    {
        ...
        left_info_ = loadCameraInfo(pkg_share + "/config/left2.yaml");
        right_info_ = loadCameraInfo(pkg_share + "/config/right2.yaml"); 
...
private:
    ...
    void imageCallback(const sensor_msgs::msg::Image::SharedPtr msg)
    {
        ...
        cv::Mat left = frame(cv::Rect(0, 0, half_width, frame.rows)).clone();
        cv::Mat right = frame(cv::Rect(half_width, 0, half_width, frame.rows)).clone();
